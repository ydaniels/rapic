...
class APIClient(APIHook, BaseClient):
    ...
    def __init__(self, client_name, request_file, **kwargs):
        ...
        self.request = RequestClient(client_name, **kwargs)
        with open(request_file, 'r') as j:
            self.client = json.loads(j.read())[client_name]
    ...
    def build_url(self, request_data, url_data, append_url):
        url = request_data.get('url')
        if not url:
            host = request_data.get('host') or self.client.get('host')
            scheme = request_data.get('scheme') or self.client.get('scheme')
            path = request_data['path'].format(url_data)
            url_query = request_data.get('url_query') or self.client.get('default_url_query')
    ...
    def execute_request(self, request_data, headers=None, url_data=None, data=None, append_url=None):
        """
        ...
        request_data = self.build_request_data(request_name, request_data, data, url_data, headers, append_url)
        request_data = self._run_hook_func(request_name, request_data, self.REQUEST_HOOK_TYPE)
        prep_req_obj = self.request.prepare_requests_request(request_data)
        new_req_obj = self._run_hook_func(request_name, prep_req_obj, self.REQUESTS_OBJ_HOOK_TYPE)
        response = self.request.execute(new_req_obj)
        response = self._run_hook_func(request_name, response, self.RESPONSE_OBJ_HOOK_TYPE)
        return response

    def build_request_data(self, request_name, request_data, data, url_data, headers, append_url):
        ...
        url_query_data = self.get_url_data(url_data, request_data)
        ...
        new_url_data = self._run_hook_func(request_name, url_query_data, self.URL_DATA_HOOK_TYPE)
        new_post_data = self._run_hook_func(request_name, body_data, self.POST_DATA_HOOK_TYPE)

        url = self.build_url(request_data, new_url_data, append_url)

        request_data['url'] = url
        request_data['headers'] = new_header
        request_data['url_data'] = new_url_data
        request_data['body_data'] = new_post_data
        if request_data.get("is_json", False):
            request_data['body_data'] = json.dumps(request_data['body_data'])
        return request_data
